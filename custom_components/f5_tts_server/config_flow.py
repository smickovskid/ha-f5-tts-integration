"""Config flow for Integration F5 TTS."""
...
STEP_USER_DATA_SCHEMA = vol.Schema(
    {
        ...
        vol.Required(CONF_PORT, description={"suggested_value": "9998"}): str,
    }
)
...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    s_client = ConnectionClient(data[CONF_HOST], data[CONF_PORT])
    await s_client.connect()
    await s_client.disconnect()
    return {"title": f"F5 TTS Server - {data[CONF_HOST]}"}


class F5TTSConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle config flow for F5 TTS"""
